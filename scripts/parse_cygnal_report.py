"""
Parse Cygnal poll report PDFs/text into WetherVane poll crosstab columns.

Cygnal (cygnalinc.com) is an IVR+SMS pollster that publishes topline and crosstab
reports for state and national races.  Their reports expose two useful structures:

1. A "Voter Profile" table with sample composition by race, gender, age, education,
   and party.  These become xt_* columns.
2. Demographic ballot crosstabs.  These become xt_vote_* two-party Democratic
   vote-share columns, including gender (xt_vote_gender_men, xt_vote_gender_women)
   which most other parsers omit.
...
ingestion when a Cygnal PDF is available.

Usage:
    uv run python scripts/parse_cygnal_report.py report.pdf
    uv run python scripts/parse_cygnal_report.py report.txt --race "2026 GA Senate" --update
"""
...
# Cygnal voter-profile label → xt_* composition column
PROFILE_LABEL_MAP = {
    "White": "xt_race_white",
    "Black": "xt_race_black",
...
# Cygnal ballot-crosstab label → xt_vote_* column
VOTE_LABEL_MAP = {
...
# Labels recognized on header lines (longer multi-word labels first so they win over fragments)
KNOWN_LABELS = [
    "Non-College",
    "Non-college",
    "College grad",
...
def _split_known_labels(line: str) -> list[str]:
    ...
    for label in KNOWN_LABELS:
        # Exclude "-" from the lookbehind so "College" never matches inside "Non-College".
...
def parse_header(text: str) -> dict[str, str | int]:
    """Extract field dates and sample size from Cygnal methodology text."""
    text = normalize_text(text)
    result: dict[str, str | int] = {}

    # Cygnal format: "N=800 Likely Voters | Conducted April 14-15, 2026"
    # Also handles: "Sample: 800 Likely Voters" (fallback Quantus-style)
    ...
    # Cygnal "Conducted Month D-D, YYYY" or "Conducted Month D-Month D, YYYY"
    conducted_match = re.search(
        r"Conducted\s+([A-Za-z]+)\s+(\d{1,2})\s*-\s*(?:([A-Za-z]+)\s+)?(\d{1,2})(?:,\s*(\d{4}))?",
        text,
        re.IGNORECASE,
    )
    if conducted_match:
    ...
    # Fallback: Quantus-style "Fielded Month D-D, YYYY"
    if "date_start" not in result:
...
def _infer_report_year(text: str) -> int:
    ...
    raise ValueError("Could not infer report year from Cygnal text")


def parse_sample_composition(text: str) -> dict[str, float]:
    """Parse Cygnal voter-profile composition into xt_* fraction columns.

    Cygnal voter-profile tables appear as a single-row header + single percentage row:
        Totals  Men  Women  White  Black  Hispanic  ...  College  Non-College  ...
        100%    48%   52%    68%    12%     13%      ...   41%       59%       ...

    The parser scans for header lines with 2+ known labels followed immediately by
    a percentage row, then zips them together.
    """
    ...
    for i, line in enumerate(lines[:-1]):
        ...
        if not values:
            continue
        # Accept "Sample (pct)" prefix rows or bare percentage rows
        bare = re.sub(r"(?:Sample\s*\(pct\)|Pct|Totals?|100%)", "", next_line, flags=re.IGNORECASE).strip()
        bare_without_pcts = re.sub(r"\d+(?:\.\d+)?\s*%", "", bare).strip()
        if bare_without_pcts and not next_line.lower().startswith("pct") and not next_line.lower().startswith("sample"):
            continue
...
def parse_demographic_vote_shares(text: str) -> dict[str, float]:
    """Parse Cygnal demographic ballot blocks into xt_vote_* columns.

    Cygnal crosstabs appear as section blocks, each with a header line listing
    demographic labels followed by labeled response rows:

        By Race
              White  Black  Hispanic  Asian
        Democrat       36%    79%     51%      44%
        Republican     55%    11%     36%      44%

    The parser handles both labeled-row format (above) and compact format
    (two bare percentage rows under a label header line).
    """
    lines = [line.strip() for line in normalize_text(text).splitlines() if line.strip()]
    result: dict[str, float] = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse Cygnal poll reports for xt_* poll crosstab fields."
    )
    ...
    extracted = parse_cygnal_report(args.path)

    print("\nExtracted Cygnal crosstab data:")
    print("-" * 50)
