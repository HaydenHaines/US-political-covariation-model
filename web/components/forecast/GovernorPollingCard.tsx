...
function coverageLabel(totalPolls: number, racesWithPolls: number, totalRaces: number): {
  ...
  if (totalPolls === 0) {
    return {
      ...
      narrative: "No state-level governor polling has been collected yet this cycle.",
    };
  }
  if (coveragePct < 0.3) {
    return {
...
function GovernorPollingCardSkeleton() {
  return (
    <section
...
/**
 * GovernorPollingCard — summary of current polling coverage for the governor forecast.
 *
 * Mirrors PollingCard (senate) but draws from useGovernorOverview.
 */
export function GovernorPollingCard() {
  const { data, error, isLoading } = useGovernorOverview();

  if (isLoading) {
    return <GovernorPollingCardSkeleton />;
  }

  // Silently hide on error — same pattern as PollingCard
  if (error || !data) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      <p className="mb-4 text-xs" style={{ color: "var(--color-text-muted)" }}>
        State-level governor polls collected for the current cycle
        {updatedLabel && (
