...
- **DRA block data is missing 2010 and most 2014 races.** Despite 2008-2024 presidential coverage in every state, `E_14_*` columns exist in only 4 states (MA/MN/NC/TX) and `E_10_*` exists in zero. Do not assume DRA replaces VEST for midterm cycles. When designing training-window extensions or off-cycle shift dims pre-2016, verify column presence per state with `head -1 election_data_block_<state>.v*.csv`. (Source: GH#95 research, 2026-04-22.)
...
- **Frontend code changes require a rebuild + service restart.** The `wethervane-frontend` service runs a pre-built Next.js standalone bundle from `web/.next/standalone/`. Merging code changes does NOT hot-reload — you must: (1) `cd web && npm run build`, then (2) `systemctl --user restart wethervane-frontend`. Without rebuilding, new components appear in the DOM skeleton but SWR hooks never fire and client components don't mount. (Source: S586, FitScoreSection invisible after merge.)
